...
public class Series {
    ...
    public static String[] series = new String[20];
    ...
    public void delete_series(int x){
        int k=x-1;
        rat=rat-Rating[k];
        for (int j = k; j<i-1; j++) {
            series[j] = series[j+1];
    ...
    public void display_series(){
        ...
        System.out.printf( "%5s|| %30s|| %20s|| %20s|| %10s||" + "", "Index", "Series Name", "Consumption(Hour)", "Consumption(Day)","Rating");
        System.out.println();

        for(int j=0;j<i;j++) {
            int k = j+1;
            System.out.format("%5s %30s %20f %20f %10s", k, series[j],Hour[j],Day[j],Rating[j]);
            System.out.println();
        }
    }
    public void show_details(int r){
        int id=r-1;
        System.out.printf("%30s %15 %15s %18s %18s %6s", "Book Name","Start_Date","End_Date", "Consumption(Hour)", "Consumption(Day)","Rating");
        System.out.println();
        System.out.format("%30s %15s %15s %18f %18f %6s", series[id],Start_date[id],End_date[id],Hour[id],Day[id],Rating[id]);
        System.out.println();
    }
