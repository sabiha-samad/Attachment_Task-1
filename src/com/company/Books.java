...
public class Books {
    ...
    public void display_book(){
        ...
        for(int j=0;j<i;j++) {
            int k = j+1;
            System.out.format("%5s %15s %20f %20f %10f", k,books[j],Hour[j],Day[j],Rating[j]);
            System.out.println();
        }
    }
    public void show_details(int p){
        ...
        System.out.println();
        System.out.format("%20s %15s %15s %18s %18s %10s", books[id],Start_date[id],End_date[id],Hour[id],Day[id],Rating[id]);
        System.out.println();
    }
